...
from xsam.charts.plot_types.efficient_frontier_time import EfficientFrontierTimeConfig, plot_efficient_frontier_time
...
def test_efficient_frontier_time_smoke() -> None:
    ...
    fig = plot_efficient_frontier_time(df, config)
...
def test_efficient_frontier_time_area_names() -> None:
    ...
    df = pd.DataFrame({"t": [1, 2, 3], "A": [0.2, 0.3, 0.5], "B": [0.8, 0.7, 0.5]})
    config = EfficientFrontierTimeConfig(y_columns=["A", "B"], x_column="t", area_names=["foo", "bar"])
    fig = plot_efficient_frontier_time(df, config)
